...
submission = 'submissionNB.csv'   # path of to be outputted submission file
...
class basic_NB(object):
    ...
    def predict(self, x, y=None):
        ...
        if y != None: # training mode
            mnb.partial_fit(x, y, [0, 1])

        p = mnb.predict_proba(x)
...
def logloss(p, y):
    ...
    p = max(min(p.all(), 1. - 10e-15), 10e-15)
